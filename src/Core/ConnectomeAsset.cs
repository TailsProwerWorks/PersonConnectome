...
namespace Mod.Core
{
    internal sealed partial class LifBrain
    {
        private sealed partial class ModAsset : IConnectomeAsset
        {
            ...
            int IConnectomeAsset.EdgeCount => EdgeCount;
            int IConnectomeAsset.OutgoingStart(int neuronId) => RowPointers[neuronId];
            int IConnectomeAsset.OutgoingEnd(int neuronId) => RowPointers[neuronId + 1];
            int IConnectomeAsset.TargetAt(int edgeIndex) => PostIndexes[edgeIndex];
            float IConnectomeAsset.WeightAt(int edgeIndex) => Weights[edgeIndex];
            ...
            internal static string ComputePayloadSha256(byte[] data)
            {
                ...
                unchecked
                {
                    for (long offset = 0; offset < paddedLength; offset += 64)
                        CompressBlock(data, offset, paddedLength, bitLength, state, words);
                }
            ...
            private static void CompressBlock(byte[] data, long offset, long paddedLength, ulong bitLength, uint[] state, uint[] words)
            {
                for (var i = 0; i < 16; i++)
                {
                    uint word = 0;
                    for (var j = 0; j < 4; j++)
                    {
                        ...
                        word = (word << 8) | ReadPaddedByte(data, position, paddedLength, bitLength);
                    }
                    words[i] = word;
                }
                ExpandMessageSchedule(words);
                CompressRounds(state, words);
            }

            private static byte ReadPaddedByte(byte[] data, long position, long paddedLength, ulong bitLength)
            {
                if (position < data.Length) return data[(int)position];
                if (position == data.Length) return 0x80;
                return position >= paddedLength - 8
                    ? (byte)(bitLength >> (int)((paddedLength - 1 - position) * 8))
                    : (byte)0;
            }

            private static void ExpandMessageSchedule(uint[] words)
            {
                for (var i = 16; i < 64; i++)
                {
                    var x = words[i - 15];
            ...
            private static void CompressRounds(uint[] state, uint[] words)
            {
