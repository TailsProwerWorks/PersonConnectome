...
namespace Mod.Adapters
{
    internal sealed class PersonConnectomeLimbController
    {
        ...
        private readonly OwnedRegenerationRate bloodRate = new();
        private readonly OwnedRegenerationRate limbRate = new();
        ...
        {
            if (limb == null) return "missing-limb";
            return StructuralFailure() ?? HealthFailure() ?? StatusFailure();
        }
        ...
        private string? StructuralFailure()
        {
            ...
            return limb.IsDismembered ? "dismembered" : null;
        }

        private string? HealthFailure()
        {
            if (!IsFinite(limb.Health) || !IsFinite(limb.InitialHealth) || limb.InitialHealth <= 0f) return "invalid-health";
            return limb.Health <= 0f ? "dead-limb" : null;
        }
        ...
        private string? StatusFailure()
        {
            ...
            return circulation.IsDisconnected || !circulation.HasCirculation ? "disconnected" : null;
        }
        ...
        private sealed class OwnedRegenerationRate
        {
            ...
            {
                if (owns && !ApproximatelyEqual(current, last)) owns = false;
            ...
            {
                var restored = owns && ApproximatelyEqual(current, last) ? baseline : current;
                owns = false;
                return restored;
            }

            private static bool ApproximatelyEqual(float left, float right) => Math.Abs(left - right) <= FloatTolerance;
        }

        private static bool ContainsAny(string value, params string[] parts)
        {
            return Array.Exists(parts, part => Contains(value, part));
        }
