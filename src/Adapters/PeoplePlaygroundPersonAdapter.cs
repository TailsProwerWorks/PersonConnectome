...
namespace Mod.Adapters
{
    ...
    /// All reflection, Unity physics, liquids, and status effects stay in this layer.
    /// </summary>
    internal sealed class PeoplePlaygroundPersonAdapter : IBodyAdapter
    {
            ...
            get
            {
                ...
                else
                {
                    ...
                    foreach (var name in names)
                    {
                        ...
                        summary.Append("\n  ").Append(name).Append(": ").Append((reading.Fraction * 100f).ToString("0.0")).Append("% (").Append(reading.Kind).Append(')');
                    }
                }
                if (invalidLiquidReadings > 0) summary.Append("\n  Incomplete: invalid/unavailable liquid readings skipped.");
        ...
        private LiveReading SelectLiveSignal()
        {
            ...
            return SelectHazardSignal() ?? SelectEnvironmentSignal() ?? SelectBodySignal() ?? SelectContactSignal() ??
                (lastFrame.Velocity > .05f ? new LiveReading("SELF-MOTION", lastFrame.Velocity) : new LiveReading("NONE", 0f));

        }
        private LiveReading? SelectHazardSignal()
        {
        ...
        private LiveReading? SelectBodySignal()
        {
        ...
        private LiveReading? SelectEnvironmentSignal()
        {
        ...
        private LiveReading? SelectContactSignal()
        {
            ...
            return lastFrame.Wetness > .05f ? new LiveReading("WETNESS", lastFrame.Wetness) : null;
        }
        ...
        public SensoryFrame Read()
        {
            ...
            var audioSourceSummary = "none";
            winningAudioSource = null;
            visionTargetSummary = "none";
            ReadLimbHealth(ref frame, out var healthSum, out var healthCount, out var trackedLimbCount, out var lostLimbCount);
            ...
            ReadNearby(ref frame, ref audioSourceSummary);
        ...
        private void ReadLimbHealth(ref SensoryFrame frame, out float healthSum, out int healthCount, out int trackedLimbCount, out int lostLimbCount)
        {
            healthSum = 0f;
            healthCount = 0;
            trackedLimbCount = 0;
        ...
        private static float GetDownwardSpeed(LimbBehaviour limb)
        {
            var body = limb?.PhysicalBehaviour?.rigidbody;
        ...
        private SensoryFrame ReadPersonState()
        {
            ...
            var body = anchor?.GetComponent<Rigidbody2D>();
            ...
            frame.AngularVelocity = body != null && frame.AngularVelocityValid ? body.angularVelocity : 0f;
            if (body != null && frame.VelocityValid)
            {
        ...
        private float ReadLimb(ref SensoryFrame frame, LimbBehaviour limb, out bool healthValid, out bool connected)
        {
            ...
            var health = GetLimbHealth(limb, healthValid);
            TrackLimbHealth(ref frame, limb, health, healthValid, connected);
            if (!connected)
            {
            ...
            ReadConnectedLimb(ref frame, limb, health, healthValid);
            return health;
        }

        private static void ReadJoint(ref SensoryFrame frame, LimbBehaviour limb, bool connected)
        {
            var joint = limb.Joint;
            if (!connected || joint == null || joint.transform != limb.transform || joint.connectedBody == null || !IsFinite(joint.jointAngle) || !IsFinite(joint.jointSpeed)) return;
            ...
            if (IsFinite(limb.JointStress)) frame.NeuralJointLoad = Mathf.Max(frame.NeuralJointLoad, Unit(limb.JointStress / 100f));
        }

        private static float GetLimbHealth(LimbBehaviour limb, bool healthValid)
        {
            if (!healthValid) return 0f;
            if (limb.Health >= limb.InitialHealth) return 1f;
            return Unit(limb.Health / limb.InitialHealth);
        }

        private void TrackLimbHealth(ref SensoryFrame frame, LimbBehaviour limb, float health, bool healthValid, bool connected)
        {
            if (healthValid && connected)
            {
                if (healthSamples.TryGetValue(limb, out var previous) && ApproximatelyEqual(previous.InitialHealth, limb.InitialHealth) && health < previous.Health)
            ...
            if (IsLostLimb(limb) && healthSamples.TryGetValue(limb, out var lostPrevious) && ApproximatelyEqual(lostPrevious.InitialHealth, limb.InitialHealth))
                frame.DamageEvent = Mathf.Max(frame.DamageEvent, lostPrevious.Health);
            healthSamples.Remove(limb);
        }

        private static bool ApproximatelyEqual(float left, float right)
        {
            return IsFinite(left) && IsFinite(right) && Mathf.Abs(left - right) <= .0001f;
        }

        private void ReadConnectedLimb(ref SensoryFrame frame, LimbBehaviour limb, float health, bool healthValid)
        {
            ...
            if (healthValid)
            {
                // Report the derived minimum health as the vitality proxy.
        ...
        private void ReadCirculation(ref SensoryFrame frame, CirculationBehaviour circulation)
        {
            ...
            ReadLiquidIdentities(ref frame, circulation, true);
        }
        ...
        private void ReadLiquidIdentities(ref SensoryFrame frame, CirculationBehaviour circulation, bool applyEffects)
        {
            if (circulation == null)
            {
                invalidLiquidReadings++;
            ...
            foreach (var entry in circulation.LiquidDistribution)
            {
                ProcessLiquidEntry(ref frame, entry.Key, entry.Value == null ? (float?)null : entry.Value.Raw, total, applyEffects);
            }
        }

        private void ProcessLiquidEntry(ref SensoryFrame frame, Liquid entry, float? rawAmount, float total, bool applyEffects)
        {
            if (entry == null || !rawAmount.HasValue || !IsFinite(rawAmount.Value) || rawAmount.Value < 0f)
            {
                invalidLiquidReadings++;
                return;
            }
            if (rawAmount.Value == 0f) return;
            if (total <= 0f || !LiquidAndStatusReader.TryNormalizeFraction(rawAmount.Value, total, out var fraction))
            {
                invalidLiquidReadings++;
                return;
            }
            var identity = Liquid.GetIdentity(entry);
            var kind = ClassifyLiquid(identity);
            var name = !String.IsNullOrEmpty(identity) ? identity : (entry.GetDisplayName() ?? "Unnamed liquid") + " (unregistered)";
            if (!liquidReadings.TryGetValue(name, out var reading) || fraction > reading.Fraction)
                liquidReadings[name] = new LiquidReading { Fraction = fraction, Kind = kind };
            if (applyEffects) ApplyLiquidEffect(ref frame, kind, fraction);
        }

        private static void ApplyLiquidEffect(ref SensoryFrame frame, LiquidKind kind, float amount)
        {
        ...
        private void ReadNearby(ref SensoryFrame f, ref string audioSourceSummary)
        {
            ...
            {
                ProcessNearbyCollider(ref f, nearbyColliders[i], scanState);
            }

            f.Light = Unit(f.AmbientLight + f.LocalLight);
            f.VisualFieldValid = f.GazeValid;
            audioSourceSummary = scanState.AudioSourceSummary;
        ...
        private void ReadVisualCandidates(ref SensoryFrame frame, Vector2 origin, Vector3 facing, Transform head, int candidateCount)
        {
            ...
            var anchorBody = head?.GetComponent<Rigidbody2D>();
            for (var i = 0; i < candidateCount; i++)
            {
                var point = visualPoints[i];
                var delta = point - origin;
                var bearing = (float)Math.Atan2(facing.x * delta.y - facing.y * delta.x, facing.x * delta.x + facing.y * delta.y) * 57.29578f;
                var band = GetVisualBand(bearing);
                if (frame.ViewAt(band).Observed || !HasExternalLineOfSight(origin, point, visualCandidates[i], ref frame)) continue;
                var collider = visualCandidates[i];
                var physical = collider.GetComponentInParent<PhysicalBehaviour>();
                var targetBody = physical?.rigidbody;
                var strength = frame.LightValid ? Mathf.Clamp01((1f - visualDistances[i] / visionRadius) * frame.Light) : 0f;
                var observation = BuildVisualObservation(collider, new VisualObservationInput(origin, delta, visualDistances[i], bearing, strength, targetBody, anchorBody));
                ...
                if (closestCollider == null)
                {
                    ...
                    frame.VisionDirection = WorldHorizontalBearing(delta, out var validDirection);
                    frame.VisionDirectionValid = validDirection;
                ...
                if (PersonConnectomeProjectileDetection.IsMovingProjectile(collider, physical)) frame.Projectile = Mathf.Max(frame.Projectile, strength);
            }
        }

        private readonly struct VisualObservationInput
        {
            public readonly Vector2 Origin, Delta;
            public readonly float Distance, Bearing, Strength;
            public readonly Rigidbody2D? TargetBody, AnchorBody;

            public VisualObservationInput(Vector2 origin, Vector2 delta, float distance, float bearing, float strength, Rigidbody2D? targetBody, Rigidbody2D? anchorBody)
            {
                Origin = origin;
                Delta = delta;
                Distance = distance;
        ...
        private static VisualObservation BuildVisualObservation(Collider2D collider, VisualObservationInput input)
        {
            var observation = new VisualObservation { Observed = input.Strength > 0f, Strength = input.Strength, BearingDegrees = input.Bearing };
            if (input.Strength <= 0f || input.TargetBody == null || input.AnchorBody == null || !IsFinite(input.TargetBody.velocity.x) || !IsFinite(input.TargetBody.velocity.y) || !IsFinite(input.AnchorBody.velocity.x) || !IsFinite(input.AnchorBody.velocity.y)) return observation;
            var closing = -((input.TargetBody.velocity.x - input.AnchorBody.velocity.x) * input.Delta.x + (input.TargetBody.velocity.y - input.AnchorBody.velocity.y) * input.Delta.y) / input.Distance;
            observation.Approach = Unit(closing / 10f) * input.Strength;
            ReadVisualGeometry(ref observation, collider, input.Origin, input.TargetBody, input.AnchorBody);
            return observation;
        }

        private void ProcessNearbyCollider(ref SensoryFrame frame, Collider2D hit, NearbyScanState state)
        {
            if (hit == null || IsOwnTransform(hit.transform)) return;
            var delta = hit.ClosestPoint(state.Origin) - state.Origin;
            var distance = delta.magnitude;
            if (!IsFinite(distance) || !IsFinite(delta.x)) return;
            ...
            if (distance < state.Closest)
            {
                ...
                frame.Nearby = Mathf.Clamp01(1f - distance / visionRadius);
                frame.NearbyDirection = Mathf.Abs(delta.x) < .001f ? 0f : Mathf.Sign(delta.x);
            }
            AddVisualCandidate(ref frame, hit, state, delta, distance);
        }

        private void ReadNearbyHazards(ref SensoryFrame frame, Collider2D hit, float distance)
        {
            var acidPool = hit.GetComponentInParent<AcidPoolBehaviour>();
            if (acidPool != null && !IsOwnTransform(acidPool.transform) && distance <= .05f)
                frame.AcidExposure = Mathf.Max(frame.AcidExposure, Mathf.Max(Unit(acidPool.AcidProgress), Unit(acidPool.PainIntensity)));
            var lava = hit.GetComponentInParent<LavaBehaviour>();
            if (lava == null || IsOwnTransform(lava.transform)) return;
            var distanceSignal = Mathf.Clamp01(1f - distance / visionRadius);
            frame.Lava = Mathf.Max(frame.Lava, distanceSignal);
        ...
        private void AddVisualCandidate(ref SensoryFrame frame, Collider2D hit, NearbyScanState state, Vector2 delta, float distance)
        {
            if (!frame.GazeValid || distance <= .001f || state.Facing.x * delta.x + state.Facing.y * delta.y <= 0f) return;
            var slot = state.CandidateCount;
            if (slot == MaxVisualCandidates)
            {
                frame.VisionLimited = true;
                if (distance >= visualDistances[slot - 1]) return;
                slot--;
            }
            ...
            visualPoints[slot] = hit.ClosestPoint(state.Origin);
        ...
        private void ReadLightSprite(ref SensoryFrame frame, SpriteRenderer renderer, Vector2 origin, float brightness)
        {
            ...
            if (!TryReadLightContribution(renderer, origin, brightness, out var contribution)) return;
            frame.LocalLight = Mathf.Max(frame.LocalLight, contribution);
            frame.LocalLightSources++;
        }

        private Vector3 InitializeNearbyState(ref SensoryFrame frame, Transform head, out Collider2D? headCollider)
        {
            var brainLimb = head?.GetComponent<LimbBehaviour>();
            frame.FoodCuesValid = pumpkinAsset != null && brainLimb != null && brainLimb.HasBrain && IsConnectedLimb(brainLimb);
            headCollider = frame.FoodCuesValid && head != null ? head.GetComponent<Collider2D>() : null;
            ...
            frame.GazeValid = brainLimb != null && brainLimb.HasBrain && IsConnectedLimb(brainLimb) && IsFinite(facing.x) && IsFinite(facing.y) && IsFinite(facingLength) && facingLength > .0001f;
            if (frame.GazeValid)
            {
                facing.x /= facingLength;
                facing.y /= facingLength;
                frame.GazeHeadingDegrees = (float)Math.Atan2(facing.y, facing.x) * 57.29578f;
            }
            return facing;
        }

        private static bool TryReadLightContribution(SpriteRenderer renderer, Vector2 origin, float brightness, out float contribution)
        {
            contribution = 0f;
            var color = renderer.color;
            if (!IsFinite(brightness) || brightness <= 0f || !IsFinite(color.r) || !IsFinite(color.g) || !IsFinite(color.b) || !IsFinite(color.a)) return false;
            var strength = Unit(color.grayscale * Unit(color.a) * brightness);
            if (strength <= 0f) return false;
            if (!TryGetLightFootprint(renderer, origin, out var point, out var centerX, out var centerY, out var extentX, out var extentY)) return false;
            var x = (point.x - centerX) / extentX;
            var y = (point.y - centerY) / extentY;
            var radiusSquared = x * x + y * y;
            if (!IsFinite(radiusSquared) || radiusSquared >= 1f) return false;
            contribution = strength * (1f - (float)Math.Sqrt(radiusSquared));
            return IsFinite(contribution) && contribution > 0f;
        }

        private static bool TryGetLightFootprint(SpriteRenderer renderer, Vector2 origin, out Vector2 point, out float centerX, out float centerY, out float extentX, out float extentY)
        {
            point = new Vector2();
            centerX = centerY = extentX = extentY = 0f;
            var scale = renderer.transform.lossyScale;
            if (!IsFinite(scale.x) || !IsFinite(scale.y) || Math.Abs(scale.x) < .00001f || Math.Abs(scale.y) < .00001f) return false;
            var bounds = renderer.sprite.bounds;
            point = (Vector2)renderer.transform.InverseTransformPoint(new Vector3(origin.x, origin.y, renderer.transform.position.z));
            if (renderer.flipX) point.x = -point.x;
            if (renderer.flipY) point.y = -point.y;
            if (!IsFinite(point.x) || !IsFinite(point.y) || !IsFinite(bounds.center.x) || !IsFinite(bounds.center.y) || !IsFinite(bounds.extents.x) || !IsFinite(bounds.extents.y) || bounds.extents.x <= 0f || bounds.extents.y <= 0f) return false;
            extentX = bounds.extents.x;
            extentY = bounds.extents.y;
            centerX = bounds.center.x;
            centerY = bounds.center.y;
            if (renderer.drawMode == SpriteDrawMode.Simple) return true;
            if (renderer.drawMode != SpriteDrawMode.Sliced && renderer.drawMode != SpriteDrawMode.Tiled) return false;
        ...
        private static int GetVisualBand(float bearing)
        {
            if (bearing < -54f) return 0;
            if (bearing < -18f) return 1;
            if (bearing < 18f) return 2;
            if (bearing < 54f) return 3;
            return 4;
        }
        ...
        private void ReadExternalAudioSource(ref SensoryFrame frame, PhysicalBehaviour physical, AudioSource audio, Vector2 origin, ref string audioSourceSummary)
        {
            if (audio == null || sampledAudioSources.Contains(audio)) return;
            if (sampledAudioSources.Count >= MaxAudioSourcesPerSample) { frame.SoundLimited = true; return; }
            sampledAudioSources.Add(audio);
            var sourceDelta = (Vector2)audio.transform.position - origin;
        ...
        private static bool IsRootName(string value)
        {
            ...
            while (index < value.Length)
            {
                ...
                if (value[index] != '(' || end < 0 || end == index + 1 || !IsValidRootSuffix(value.Substring(index + 1, end - index - 1))) return false;
                index = end + 1;
            }
            return true;
        }
