...
The reference walking weights are DNp09 0.30, DNg100 0.25, DNge053 0.15, DNge050 0.15 and DNg97 0.15. Our input to the decoder is fired/member fraction, not the reference's Hz readout. The Overview page exposes the fly request channels directly: forward, yaw, backward, halt, brake, jump, takeoff, landing, flight power/yaw, wing motor, grooming, feed, courtship, song and leg-motor activity. Forward, backward and side-specific leg channels use exponential smoothing with a 150 ms time constant. Turning combines DNa02 R-L + 0.5*DNg13 R-L + 0.25*DNa01 R-L with a 100 ms filter. The existing human body adapter consumes only a separate locomotor adaptation; it does not rename these fly channels as human joints.
...
Raw DNg60/DNg74_a/DNg74_b or AN19A018 fraction >=0.2 clears the Human adaptation immediately, as do unsafe/terminal control states. These thresholds and the floor are local engineering choices. Fly leg motor subclasses `fl`, `ml`, `hl` contribute side-specific locomotor readout (381 total); wing, song, grooming, courtship and proboscis populations remain visible as their own fly channels. MN9 is reported as `feed`, not a human grip request.
