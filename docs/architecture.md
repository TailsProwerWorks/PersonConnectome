...
At each control tick, the adapter reconciles surviving/new limbs and builds a fresh bounded sensory frame. Heuristic sensory populations receive weighted game signals; nearby direction and liquid healing feed the command decoder only. Vision is a bounded line-of-sight/light proxy, while vibration, proprioception, falling and projectile awareness are bounded mechanical/body-state channels. Falling reads downward velocity from each tracked limb's native physical body and is suppressed by floor contact; it is not fabricated from horizontal walking or unrelated child bodies. Visual target labels are conservative metadata, not semantic recognition. Sparse LIF processing caps work at 24,000 active neurons, rotates overloaded work, merges carried input with new propagation and expires refractory periods by neural tick. Telemetry exposes queued input targets, active/processed/deferred work, spikes and whether the scheduler is steady or backlogged. This bounds neuron work, not a guaranteed frame-time budget or biological fidelity.
