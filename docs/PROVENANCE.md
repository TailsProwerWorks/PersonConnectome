...
The 2026-09-14 scheduler follow-up preserves this asset byte-for-byte. It integrates all eligible active state before bounded propagation, so traces previously altered by dropped input integrations change. This is an execution-policy correction toward the existing discrete model, not new biological validation; see [scheduler-performance.md](scheduler-performance.md).

Local lamp sensing reads supported native light-sprite state into the existing scalar light channel. It changes neither the MaleCNS asset nor population selectors and is an engineered game-light proxy, not added retinal physiology. See [local-light-sensing.md](local-light-sensing.md).

Head-relative vision update (2026-09-14): the orientation/sensing feedback pattern was compared with upstream [WorldSenses.java](https://github.com/blendi-remade/fly-brain-minecraft/blob/main/src/main/java/com/fruitfly/entity/WorldSenses.java) and [FlyBody.java](https://github.com/blendi-remade/fly-brain-minecraft/blob/main/src/main/java/com/fruitfly/entity/FlyBody.java). The local implementation uses a bounded 2D frontal field and existing neural head requests. The upstream optional random exploration fallback was not imported. No connectome, soma, neuron membership or asset bytes changed. See [implementation and limitations](head-relative-vision.md).

The same update extends sensing to five 2D visual bands using existing native bounds/velocity/linecast data. Per-side maximum pooling and the 16-candidate budget are local engineering choices, not imported biological retinal geometry. No synaptic plasticity or learning has been introduced.

### Food mapping and escape actuator adaptation (2026-09-14)

The upstream [OdorTable.java](https://github.com/blendi-remade/fly-brain-minecraft/blob/main/src/main/java/com/fruitfly/entity/OdorTable.java) and [TasteTable.java](https://github.com/blendi-remade/fly-brain-minecraft/blob/main/src/main/java/com/fruitfly/entity/TasteTable.java) provide hand-built Minecraft item-to-population conventions. Our stock-Pumpkin proximity mapping to ORN_DM1/DM2/VA2 and head-contact mapping to LB3b/c are local gameplay choices using those population families, not a measured Pumpkin odor/taste signature (upstream includes pumpkin pie, not this raw stock item). No upstream nutrition/eating or fly-proboscis-to-human-grip behavior is imported. The 0.6-second qualified-DNp01 walking burst is also a local Human actuator adaptation. Existing MIT notice and MaleCNS CC BY 4.0 attribution remain; graph, weights, carrier and soma coordinates are unchanged by this follow-up.
