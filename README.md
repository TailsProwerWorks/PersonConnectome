...
- **Brain:** neural workload, derived population drives, spikes per processed tick and a soma activity map. The map displays every actual source neuron with finite soma coordinates. Colored flashes mark neurons that fired in the displayed capture; dark neurons do not establish inactivity. The X/Z projection preserves relative source coordinates and is not a human brain image.
...
- **Vision:** nearest external physical collider, line of sight and ambient light. Valid collider bounds and relative velocity provide approximate angular size, expansion and motion cues; native body rotation provides a separate visual-motion proxy under light. Target names are conservative component classifications, not recognition or semantic eyesight. A 128-collider overlap buffer can miss objects in crowded scenes.
