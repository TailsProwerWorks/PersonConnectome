...
For native standing experiments, the spawned controller exposes an opt-in `JointAwareStandingControllerEnabled` setting. When enabled, the Human adapter adds bounded per-joint posture feedback from torso tilt/velocity, joint state and support contact between neural ticks. This is a deterministic, parameterized PD-style assist for actuator validation; no optimizer or trained policy is shipped yet, and native physics testing is required before using it as a balance solution.

The Stimulation page also includes an opt-in Teach mode. Start/retry a short trial, then use Good/Bad feedback to adjust the bounded standing parameters; the status now reports a live standing score based on uprightness, supported height, motion and leg contact. Undo, Pause learning, Restore best, Reset skill, and Save/Load profile are available. Training is limited to one active person, uses one local profile slot for this first version, is stored as validated parameters rather than executable code, and does not rewrite the fly connectome or inject neural stimuli.
