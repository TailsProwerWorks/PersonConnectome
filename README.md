...
Outside explicit Teach mode, the shared topology and base connection weights stay fixed. Recent inputs affect short-term neural state, but the controller does not improve simply by staying alive. A per-person learned profile can add bounded efficacy deltas to selected existing edges; no separate Human action policy is trained or replayed.
