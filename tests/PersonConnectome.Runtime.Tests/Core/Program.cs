...
using ShadowNineX.PersonConnectome.Tests;
...
var tests = new (string Name, Action Run)[]
{
    ...
    ("default feedback catalog covers fly sensory routes", DefaultFeedbackCatalogCoversFlySensoryRoutes),
    ...
    ("remembered object threat reaches ordinary visual danger routes", LearnedObjectThreatRoutes),
    ("strong neural visual danger qualifies a bounded escape reflex", StrongVisualDangerQualifiesBoundedEscape),
...
static void StrongVisualDangerQualifiesBoundedEscape()
{
    ...
    var loomingCommand = looming.Step(danger);
    Equal(1f, loomingCommand.FlyEscape);
    Equal(0f, loomingCommand.FlyForward);
    Equal(0f, looming.Step(danger).FlyEscape);
    looming.Step(Healthy(), 2f);
    Equal(1f, looming.Step(danger).FlyEscape);

    var projectile = LifBrain.CreateForTest(1, new int[2], [], []);
    projectile.SetTestPopulation(TypeLPLC2, 0);
    Equal(1f, projectile.Step(Healthy() with { Projectile = .8f }).FlyEscape);
...
static void LearnedObjectThreatRoutes()
{
    var brain = SensoryFeatureFixture();
    brain.Step(Healthy() with
    {
        LearnedThreatValid = true,
        LearnedThreat = .5f,
        LearnedThreatDirection = -1f
    });
    True(brain.TestPotentialValue(7) > 0f, "remembered threat did not reach LC4");
    True(brain.TestPotentialValue(8) > 0f, "remembered threat did not reach LPLC2");
    True(brain.DisplayInputSummary.Contains("effective-visual-threat=0.50"));

    brain = SensoryFeatureFixture();
    brain.Step(Healthy() with { LearnedThreat = 1f, LearnedThreatDirection = 1f });
    Equal(0f, brain.TestPotentialValue(7));
    Equal(0f, brain.TestPotentialValue(8));

    brain = SensoryFeatureFixture();
    brain.Step(Healthy() with { LearnedThreatValid = true, LearnedThreat = float.NaN });
    Equal(0f, brain.TestPotentialValue(7));
    Equal(0f, brain.TestPotentialValue(8));
}
...
static void BundledPayloadIdentity()
{
    ...
    Console.WriteLine("PLASTIC catalog selected=" + brain.Plasticity.SelectedEdgeCount + " rule=" + RewardModulatedPlasticity.RuleName);
...
static void DefaultFeedbackCatalogCoversFlySensoryRoutes()
{
    var asset = new FeedbackCatalogAsset();
    var first = RewardModulatedPlasticity.SelectDefaultEdgesForTest(asset, 5);
    var second = RewardModulatedPlasticity.SelectDefaultEdgesForTest(asset, 5);
    Equal(5, first.Length);
    Equal(String.Join(",", first), String.Join(",", second));
    True(first.Contains(0), "light route was not selected");
    True(first.Contains(1), "looming visual route was not selected");
    True(first.Contains(2), "auditory route was not selected");
    True(first.Contains(3), "posture route was not selected");
    True(first.Contains(4), "fly motor route was not selected");

    var rewarded = new RewardModulatedPlasticity(asset, 5);
    rewarded.SetMode(ConnectomeLearningMode.PlasticConnectome);
    rewarded.RecordPresynaptic(1);
    rewarded.RecordPostsynaptic(5);
    True(rewarded.ApplyRewardImpulse(1f) > 0);
    True(rewarded.EffectiveWeight(1, 1f, 1) > 1f, "Good feedback did not strengthen the eligible fly visual edge");

    var discouraged = new RewardModulatedPlasticity(asset, 5);
    discouraged.SetMode(ConnectomeLearningMode.PlasticConnectome);
    discouraged.RecordPresynaptic(1);
    discouraged.RecordPostsynaptic(5);
    True(discouraged.ApplyRewardImpulse(-1f) > 0);
    True(discouraged.EffectiveWeight(1, 1f, 1) < 1f, "Bad feedback did not weaken the eligible fly visual edge");
}
...
namespace ShadowNineX.PersonConnectome.Tests
{
    internal sealed class FeedbackCatalogAsset : IConnectomeAsset
    {
        private readonly Dictionary<string, int[]> populations = new(StringComparer.OrdinalIgnoreCase)
        {
            ["input:light"] = new[] { 0 },
            ["type:LC4"] = new[] { 1 },
            ["input:auditory"] = new[] { 2 },
            ["input:gravity"] = new[] { 3 },
            ["motor:leg"] = new[] { 4 }
        };
        private static readonly int[] Empty = Array.Empty<int>();
        private static readonly int[] Rows = { 0, 1, 2, 3, 4, 5, 5 };

        public int NeuronCount => 6;
        public int EdgeCount => 5;
        public IReadOnlyList<int> Population(string name) => populations.TryGetValue(name, out var ids) ? ids : Empty;
        public int OutgoingStart(int neuronId) => Rows[neuronId];
        public int OutgoingEnd(int neuronId) => Rows[neuronId + 1];
        public int TargetAt(int edgeIndex) => 5;
        public float WeightAt(int edgeIndex) => 1f;
        public sbyte SignAt(int neuronId) => 1;
        public string SuperclassAt(int neuronId) => String.Empty;
        public string SideAt(int neuronId) => String.Empty;
        public void SetPopulation(string name, int[] ids) => populations[name] = ids;
    }
